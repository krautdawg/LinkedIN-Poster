...
def check_environment():
    ...
    if not all([
            Config.OPENAI_API_KEY, Config.NEWS_API_KEY,
...
class NewsCollector:
    ...
    def get_recent_news() -> List[Dict]:
        ...
        priority_1_articles = newsapi.get_everything(q=priority_1_query,
                                                     language='de',
                                                     sort_by='relevancy',
                                                     page_size=20,
        ...
        print(
            f"Total articles found after filtering: {len(filtered_articles)}")
...
class ContentGenerator:
    ...
    def _generate_post_content(content: str) -> str:
        ...
        stored_posts_text = "\n\n".join([
            post.get('content', '') for post in stored_posts[-30:]
        ])  # Last 30 posts
    ...
    async def send_to_telegram(posts: Dict) -> None:
        ...
        try:
            ...
            bot = Application.builder().token(
                Config.TELEGRAM_BOT_TOKEN).connection_pool_size(
                    8).pool_timeout(30.0).connect_timeout(30.0).read_timeout(
                        30.0).write_timeout(30.0).build()
            ...
            unique_posts = [
                ...
                if not PostDatabase.is_duplicate_article(post['sourceUrl'])
            ]
            ...
            for i, post in enumerate(unique_posts, 1):
                try:
                    ...
                    content = post['content'].replace('_', '\\_').replace(
                        '*', '\\*').replace('`', '\\`').replace('[',
                                                                '\\[').replace(
                                                                    ']', '\\]')
                    # Escape URL special characters
                    url = post['sourceUrl'].replace('_', '\\_').replace(
                        '*', '\\*').replace('`', '\\`').replace('[',
                                                                '\\[').replace(
                                                                    ']', '\\]')
...
"""
                    ...
                    await bot.bot.send_message(chat_id=Config.TELEGRAM_CHAT_ID,
                                               text=message,
                                               parse_mode='Markdown',
            ...
            if "chat not found" in str(e).lower():
                print(
                    f"Please check if the TELEGRAM_CHAT_ID ({Config.TELEGRAM_CHAT_ID}) is correct"
                )
    ...
    async def post_to_linkedin(post_content: str,
        ...
        try:
            ...
            response = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: requests.post("https://api.linkedin.com/v2/ugcPosts",
                                      headers=headers,
                                      json=payload))
            ...
            if response.status_code == 201:
                ...
                print(
                    f"Failed to post to LinkedIn. Status Code: {response.status_code}"
                )
...
async def handle_selection(update, context):
    ...
    if not SocialMedia.stored_posts:
        await update.message.reply_text(
            "No articles available. Please wait for the next update.")
        return
    ...
    try:
        ...
        if 1 <= selection <= 7:
            ...
            title = next((article['title'] for article in articles
                          if article['url'] == selected_post['sourceUrl']),
                         "AI News Article")
            result = await SocialMedia.post_to_linkedin(
                post_content, selected_post['sourceUrl'], title)
            if result == True:
                from db_manager import PostDatabase
        ...
        await update.message.reply_text(
            "Please send a number between 1 and 3 to select an article.")
